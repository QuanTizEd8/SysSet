...
get_matching_github_ref() {
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      --owner) shift; owner="$1"; echo "📩 Read argument 'owner': '${owner}'" >&2; shift;;
      --ref) shift; ref="$1"; echo "📩 Read argument 'ref': '${ref}'" >&2; shift;;
      --regex) shift; regex="$1"; echo "📩 Read argument 'regex': '${regex}'" >&2; shift;;
      --remove_prefix) shift; remove_prefix="$1"; echo "📩 Read argument 'remove_prefix': '${remove_prefix}'" >&2; shift;;
      --repo) shift; repo="$1"; echo "📩 Read argument 'repo': '${repo}'" >&2; shift;;
      --help|-h) __usage__;;
      --*) echo "⛔ Unknown option: '${1}'" >&2; exit 1;;
      *) echo "⛔ Unexpected argument: '${1}'" >&2; exit 1;;
    esac
  done
  [ -z "${owner-}" ] && { echo "⛔ Missing required argument 'owner'." >&2; exit 1; }
  [ -z "${ref-}" ] && { echo "⛔ Missing required argument 'ref'." >&2; exit 1; }
  [ -z "${regex-}" ] && { echo "ℹ️ Argument 'regex' set to default value ''." >&2; regex=""; }
  [ -z "${remove_prefix-}" ] && { echo "ℹ️ Argument 'remove_prefix' set to default value ''." >&2; remove_prefix=""; }
  [ -z "${repo-}" ] && { echo "⛔ Missing required argument 'repo'." >&2; exit 1; }
...
[ -z "${INSTALLER_DIR-}" ] && { echo "ℹ️ Argument 'INSTALLER_DIR' set to default value '/tmp/git-installer'." >&2; INSTALLER_DIR="/tmp/git-installer"; }
...
[ -n "${SYSPKG_INSTALL_SCRIPT-}" ] && [ ! -x "${SYSPKG_INSTALL_SCRIPT}" ] && { echo "⛔ Executable argument to parameter 'SYSPKG_INSTALL_SCRIPT' not found: '${SYSPKG_INSTALL_SCRIPT}'" >&2; exit 1; }
