...
def generate_block(feature_name: str, options: dict, dependencies: dict | None = None) -> str:
    ...
    If ``dependencies.run`` contains a ``base`` key, a ``_base_deps__install``
    call is emitted as the last statement before the end marker.
    If ``dependencies.build`` is non-empty, ``_all_build_deps__cleanup`` is
    wired into the ``_on_exit`` success branch and helper functions are emitted.
    """
    # ── Derive has_ospkg, has_build_deps and build the final options dict ─────
    ...
    has_build_deps: bool = bool(build_deps)
    ...
    keys_to_strip = (
        always_strip
        | ({"keep_cache"} if has_ospkg else set())
        | ({"keep_build_deps"} if has_build_deps else set())
    )
    core_options: dict = {
    ...
    if has_build_deps:
        final_options["keep_build_deps"] = _synthetic("keep_build_deps")
    ...
    # ── dep group helpers ─────────────────────────────────────────────────────
    # Emit _<group>_deps__install() for every run group (auto-called in header
    # for "base"; all others are called explicitly from the feature body).
    # Emit _<group>_deps__install() / _<group>_deps__cleanup() for every build
    # group (NEVER auto-called — always called explicitly from the feature body).
    # Emit _all_build_deps__cleanup() wrapper when there are any build groups.
    if run_deps or build_deps:
        lines.append("")
        lines.append("# ── dep-group helpers (generated) ──────────────────────────────────────────────")
        for group_name in run_deps:
            _safe = group_name.replace("-", "_")
            lines.append(f"# shellcheck disable=SC2329,SC2317")
            lines.append(f"_{_safe}_deps__install() {{")
            lines.append(
                f'  ospkg__run --manifest "${{_BASE_DIR}}/dependencies/run/{group_name}.yaml" --skip_installed'
            )
            lines.append("  return")
            lines.append("}")
        for group_name in build_deps:
            ...
            lines.append(f"_{_safe}_deps__install() {{")
            lines.append(
                f'  ospkg__run --manifest "${{_BASE_DIR}}/dependencies/build/{group_name}.yaml" --skip_installed --build-group "{feature_name}-{group_name}"'
            )
            lines.append("  return")
            lines.append("}")
            lines.append(f"# shellcheck disable=SC2329,SC2317")
            lines.append(f"_{_safe}_deps__cleanup() {{")
            lines.append(
                f'  ospkg__run --remove-build-group "{feature_name}-{group_name}"'
            )
            lines.append("  return")
            lines.append("}")
        if has_build_deps:
            lines.append("# shellcheck disable=SC2329,SC2317")
            lines.append("_all_build_deps__cleanup() {")
            for group_name in build_deps:
                _safe = group_name.replace("-", "_")
                lines.append(f"  _{_safe}_deps__cleanup")
            lines.append("  return")
