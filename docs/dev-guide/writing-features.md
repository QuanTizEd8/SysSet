...
2. Create `features/<feature-id>/metadata.yaml` following the structure
   described in [copilot-instructions.md](../../.github/copilot-instructions.md).
2. Create `features/<feature-id>/install.bash` (body only — no header;
   the header is generated by `sync-lib.sh`) following the structure
   described below.
3. Run `bash sync-lib.sh` — this assembles `src/<feature-id>/install.bash`,
   generates `src/<feature-id>/install.sh` (bootstrap) and `src/<feature-id>/_lib/`.
...
5. If the feature requires OS packages before `install.bash` runs,
   add a `dependencies.base` key to `features/<feature-id>/metadata.yaml`.
...
```
features/<feature-id>/
├── metadata.yaml               ← Metadata, options, dependencies (source of truth)
├── install.bash                ← Body-only installer (header generated by sync)
└── files/                      ← Optional static files
```

After running `bash sync-lib.sh`, the assembled feature is in `src/<feature-id>/` (git-ignored):
...
├── dependencies/               ← Generated from metadata.yaml
...
`_SELF_DIR` and `_BASE_DIR` both resolve to `src/<feature>/` (in the assembled output).
Use them to source `_lib/`,
...
Create a `dependencies.base` key in `features/<feature>/metadata.yaml` when the installer needs tools
(e.g. `curl`, `ca-certificates`) to be present before any other work begins.
`sync-deps.py` converts this to `src/<feature>/dependencies/base.yaml` automatically.
The format is a YAML ospkg manifest (see
...
container, place them under `features/<feature>/files/`. Reference them in
`install.bash` via `_FILES_DIR="${_BASE_DIR}/files"`.
During sync, `files/` is copied to `src/<feature>/files/` (not symlinked).
