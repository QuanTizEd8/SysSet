...
**Feature keys** in `"features"` must be the **directory name under `src/`** — the CLI uses the key as a filesystem path to locate and copy the feature source. This is independent of the feature's `"id"` field in `devcontainer-feature.json`. For example, the directory `install-conda-env` has `"id": "conda-env"`, but the key in `"features"` is `"install-conda-env"`.
