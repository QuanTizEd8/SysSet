...
When adding a new feature, no CI changes are needed —
the discovery job picks it up automatically once a `devcontainer-feature.json` and `test/` directory exist.

**Trigger manually with `gh`:** To run all features without waiting for a push, dispatch `workflow_dispatch` from the CLI:

```bash
gh workflow run "CI - Test Features"
```

This tests every feature under `.devcontainer/src/`. To watch the run as it progresses:
