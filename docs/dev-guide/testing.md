...
```bash
devcontainer features test -f install-miniforge --skip-autogenerated --project-folder . .
devcontainer features test -f install-conda-env --skip-autogenerated --project-folder . .
devcontainer features test -f setup-user        --skip-autogenerated --project-folder . .
devcontainer features test -f install-os-pkg    --skip-autogenerated --project-folder . .
```
...
No `scenarios.json` entry is needed — `run-macos.sh` discovers scripts directly from `test/<feature>/macos/*.sh`. CI runs these via `test-macos.yaml` on a dedicated `macos-latest` job.

---

## CI

Two GitHub Actions workflows handle feature tests:

**`test.yaml` — Linux feature scenario tests.** A `discover` job lists all features under `src/`. On `push` or `pull_request`, only features whose files changed (under `src/<feature>/` or `test/<feature>/`) are tested. On `workflow_dispatch` (manual trigger), all features are tested. A matrix `test` job runs per discovered feature:

```
devcontainer features test -f <feature> --skip-autogenerated --project-folder . .
```

Each `test` job also runs `bash test/run-fail-scenarios.sh <feature>` for any feature that has a `fail_scenarios.sh`.

**`test-macos.yaml` — macOS feature scenarios.** Discovers features that have at least one `test/<feature>/macos/*.sh` file. On push/PR, only changed features with macOS scenarios are tested. On `workflow_dispatch`, all features with macOS scenarios are tested. Each discovered feature runs `bash test/run-macos.sh <feature>` on a `macos-latest` runner.

**Manifest resolution tests:** A separate `test-install-os-pkg` matrix job in `test.yaml` runs the dry-run suite across 6 distro images whenever `install-os-pkg` is in the discovered set.

When adding a new feature, no CI changes are needed — discovery is automatic once a `devcontainer-feature.json` and `test/` directory exist. When adding macOS scenarios, no CI changes are needed — `test-macos.yaml` discovers them automatically.

**Trigger manually with `gh`:**

```bash
gh workflow run "CI - Test Features"
gh workflow run "CI - Test Features (macOS)"
```

To watch the run as it progresses:
...
```bash
gh run list --workflow "CI - Test Features"
```
...
`.github/workflows/test-unit.yaml` runs the full suite on every push or PR that touches `lib/**` or `test/unit/**` across three job groups:
...
The GHA `macos-latest` runner does **not** have bash ≥4 pre-installed; `test-unit.yaml` adds an explicit `brew install bash` step before running the suite. `test/run-unit.sh` handles the re-exec automatically for local runs.

```bash
# Trigger all unit tests manually
