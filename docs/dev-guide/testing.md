...
**Discovery:** A `discover` job lists all features under `src/`. On `push` or `pull_request`, only features whose files changed (under `src/<feature>/` or `test/<feature>/`) are tested. On `workflow_dispatch` (manual trigger), all features are tested.
...
|---|---|---|
| `os.bats` | `os.sh` | 25 |
| `shell.bats` | `shell.sh` | 22 |
| `ospkg.bats` | `ospkg.sh` | 18 |
| `logging.bats` | `logging.sh` | 6 |
| `net.bats` | `net.sh` | 6 |
| `git.bats` | `git.sh` | 6 |
| `checksum.bats` | `checksum.sh` | 6 |
| `github.bats` | `github.sh` | 8 |
| `users.bats` | `users.sh` | 9 |
...
`.github/workflows/test-unit.yaml` runs the full suite on every push or PR
that touches `lib/**` or `test/unit/**`, using a matrix of
...
A lefthook **pre-push** hook also runs the unit suite locally whenever
`lib/` or `test/unit/` files are changed, catching regressions before the
push reaches CI.
