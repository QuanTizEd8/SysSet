...
| `if_exists` | string | `"skip"` | What to do when a pixi binary is already found at `$prefix/bin/pixi`. One of: `"skip"` (warn and continue), `"fail"` (exit non-zero), `"reinstall"` (remove then install fresh), `"update"` (run `pixi self-update --version <resolved_version>`). Version-match: if installed version equals resolved target, always skips silently. |
...
| `symlink` | boolean | `true` | Create a symlink from the canonical bin directory to `$PREFIX/bin/pixi` when `prefix` resolves to a non-default path. Root: `/usr/local/bin/pixi → $PREFIX/bin/pixi`. Non-root: `$HOME/.pixi/bin/pixi → $PREFIX/bin/pixi`. No-op when `prefix` already resolves to the canonical path (`/usr/local` for root, `$HOME/.pixi` for non-root). |
