...
| `add_current_user_config` | boolean | `true` | Apply per-user git configuration (git_protocol, setup_git, sign_commits) and install extensions for the current non-root user (SUDO_USER if run via sudo, otherwise the result of whoami). No effect when the current user is root and other non-root users are also being targeted. Only applies when at least one of 'extensions', 'git_protocol', 'setup_git', or 'sign_commits' is active. |
| `add_remote_user_config` | boolean | `true` | Apply per-user git configuration (git_protocol, setup_git, sign_commits) and install extensions for the devcontainer remoteUser (the _REMOTE_USER environment variable set by the devcontainer tooling). No effect when _REMOTE_USER is not set. Only applies when at least one of 'extensions', 'git_protocol', 'setup_git', or 'sign_commits' is active. |
| `add_container_user_config` | boolean | `true` | Apply per-user git configuration (git_protocol, setup_git, sign_commits) and install extensions for the devcontainer containerUser (the _CONTAINER_USER environment variable set by the devcontainer tooling). No effect when _CONTAINER_USER is not set. Only applies when at least one of 'extensions', 'git_protocol', 'setup_git', or 'sign_commits' is active. |
...
| `git_protocol` | string (proposals) | `""` | Set the default git protocol in gh's per-user config (~/.config/gh/config.yml) by running 'gh config set git_protocol <value>' for each resolved user. 'https' — use HTTPS for git clone and push operations (gh's built-in default). 'ssh' — use SSH for git clone and push. '' (empty, default) — leave the config key unset; gh uses its own default (https). Applied for all users resolved by the add_*_user_config / add_user_config options. |
