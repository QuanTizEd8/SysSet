...
# ── Private: YAML manifest format detection ───────────────────────────────────
# _ospkg_is_yaml_manifest <content>
# Returns 0 when content looks like a YAML/JSON manifest (new format),
# 1 when it looks like a text DSL manifest (old format).
_ospkg_is_yaml_manifest() {
  local _content="$1"
  # JSON: starts with {
  [[ "${_content:0:1}" == "{" ]] && return 0
  # Text DSL: section header --- type [selectors]
  if echo "$_content" | grep -qE '^--- (pkg|key|prescript|repo|script|group|module)'; then
    return 1 # text DSL
  fi
  # Old-style per-line selectors [key=val] on bare package lines.
  if echo "$_content" | grep -qE '^\S+\s+\[.+=.+\]'; then
    return 1 # text DSL
  fi
  # YAML: known top-level keys
  if echo "$_content" | grep -qE '^(packages|apt|apk|brew|dnf|yum|zypper|pacman|prescripts|scripts):'; then
    return 0 # YAML
  fi
  # Default: treat bare package lists as text DSL for backward compatibility.
  return 1
}
...
ospkg::run() {
  ...
  # ── Route to format-appropriate parser ────────────────────────────────────
  if [[ -n "$_manifest_content" ]] && _ospkg_is_yaml_manifest "$_manifest_content"; then
    # ── YAML / JSON manifest path ──────────────────────────────────────────
    echo "ℹ️  Manifest format detected: YAML/JSON." >&2
    ...
    # ── Text DSL manifest path ─────────────────────────────────────────────
    [[ -n "$_manifest_content" ]] && echo "ℹ️  Manifest format detected: text DSL." >&2
    ...
    if [[ -n "$_M_KEY" ]]; then
      ...
      if [[ "$_dry_run" == true ]]; then
        ...
        local _kline_dsl _kurl_dsl _kdest_dsl
        while IFS= read -r _kline_dsl || [[ -n "$_kline_dsl" ]]; do
          [[ -z "${_kline_dsl:-}" ]] && continue
          _kurl_dsl="${_kline_dsl%% *}"
          _kdest_dsl="${_kline_dsl#* }"
          echo "    $_kurl_dsl → $_kdest_dsl" >&2
        done <<< "$_M_KEY"
      else
        export GNUPGHOME="$_key_gnupghome"
        local _kline_dsl _kurl_dsl _kdest_dsl
        while IFS= read -r _kline_dsl || [[ -n "$_kline_dsl" ]]; do
          ...
          _kurl_dsl="${_kline_dsl%% *}"
          _kdest_dsl="${_kline_dsl#* }"
          _ospkg_install_key_entry "$_kurl_dsl" "$_kdest_dsl"
        done <<< "$_M_KEY"
        unset GNUPGHOME
      fi
      rm -rf "$_key_gnupghome"
    ...
    if [[ -n "$_M_REPO" ]]; then
      ...
      if [[ "$_dry_run" == true ]]; then
        echo "🔍 [dry-run] repo: $(echo -n "$_M_REPO" | wc -l | tr -d ' ') line(s) — would add." >&2
      else
    ...
    if [[ -n "$_M_PKG" && "$_no_update" == false ]]; then
      if [[ "$_dry_run" == true ]]; then
        ...
        local _dsl_update_args=(--lists_max_age "$_lists_max_age")
        [[ "$_repo_added" == true ]] && _dsl_update_args+=(--repo_added)
        ospkg::update "${_dsl_update_args[@]}"
      fi
    elif [[ -z "$_M_PKG" ]]; then
      echo "ℹ️  Package list update skipped (no packages in manifest)." >&2
    else
      ...
      if [[ "$_repo_added" == true ]]; then
        echo "⚠️  A repository was added but --no_update is set." >&2
      fi
    fi

    # Enable DNF module streams.
    if [[ -n "$_M_MODULE" ]]; then
      if [[ "$_OSPKG_PREFIX" = "dnf" ]]; then
  ...
  fi # end format routing

  # ── Cache cleanup (both paths) ────────────────────────────────────────────
  if [[ "$_dry_run" == true ]]; then
