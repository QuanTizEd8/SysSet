...
# checksum::verify_sha256 <file> <expected_hash>
#
...
checksum::verify_sha256() {
  ...
  if command -v sha256sum > /dev/null 2>&1; then
    ...
    echo "⛔ checksum::verify_sha256: neither sha256sum nor shasum is available." >&2
    return 1
  fi
...
# checksum::verify_sha256_sidecar <file> <sha256_file>
#
# Reads the first whitespace-separated field of <sha256_file> as the expected
# hash, then delegates to checksum::verify_sha256.
...
checksum::verify_sha256_sidecar() {
  ...
  [ -z "$_expected" ] && {
    echo "⛔ checksum::verify_sha256_sidecar: could not read hash from '${_sha256_file}'." >&2
    return 1
  }
