...
tools: [execute, read, edit, search, web, agent, todo, mcp_github/*, mcp_io_github_git/*]
...
3. Compile findings into a comprehensive "Developer Notes" section (always the last section in the doc, before References) at `docs/ref/<feature-name>.md`.
...
2. Design a comprehensive, flexible API (the `options` object in `devcontainer-feature.json`) that covers:
...
The auditor will read all code from disk, find flaws, write tests, run them, and return a structured report. **Do NOT proceed to Phase 5 until you have addressed every CRITICAL issue in the auditor's report.** Warnings may be deferred but must be acknowledged.

When fixing issues found by the auditor:
1. Fix the root cause in the implementation — never in the tests.
...
2. Re-invoke the final reviewer.
...
- **bootstrap.sh**: POSIX sh wrapper that finds bash ≥ 4 and execs `scripts/install.sh`.
