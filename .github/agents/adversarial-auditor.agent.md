...
Based on findings from Step 2, write tests in three categories:

1. **Unit tests** (`test/unit/<module>.bats`) — for any `lib/` function that was modified or is used in a fragile way. Read `test/unit/helpers/common.bash` and existing `.bats` files first to understand the framework.

2. **Scenario tests** (`test/<feature>/scenarios.json` + `<scenario>.sh`) — for devcontainer feature behavior. Read existing `scenarios.json` files for format reference.
