...
Run with: `bash test/run-fail-scenarios.sh <feature>`
...
- **`test.sh` is a required stub.** The CLI looks for it even with `--skip-autogenerated`. Never put real assertions in it — all coverage goes in `scenarios.json` scenarios.
...
- **Always pass `--skip-autogenerated`.** All features stub `test.sh` to skip the autogenerated build.
